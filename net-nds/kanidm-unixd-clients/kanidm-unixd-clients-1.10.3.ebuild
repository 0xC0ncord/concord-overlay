...
unixd_maybe_restart() {
	...
	if [[ ${ROOT} ]]; then
		...
		ewarn "The ebuild will now attempt to restart the Kanidm Unix"
		ewarn "resolver to avoid bricking the running instance."

		ebegin "Attempting to restart kanidm-unixd via 'systemctl try-restart kanidm-unixd'"
		systemctl try-restart kanidm-unixd
		eend $?
		ebegin "Attempting to restart kanidm-unixd-tasks via 'systemctl try-restart kanidm-unixd-tasks'"
		systemctl try-restart kanidm-unixd-tasks
		eend $?
	elif [[ -d /run/openrc ]]; then
		ewarn "The ebuild will now attempt to restart the Kanidm Unix"
		ewarn "resolver to avoid bricking the running instance."

		ebegin "Attempting to restart openssh via 'rc-service -q --ifstarted --nodeps kanidm-unixd restart'"
		rc-service -q --ifstarted --nodeps kanidm-unixd restart
		eend $?
		ebegin "Attempting to restart openssh via 'rc-service -q --ifstarted --nodeps kanidm-unixd-tasks restart'"
		rc-service -q --ifstarted --nodeps kanidm-unixd-tasks restart
		eend $?
	fi
}
