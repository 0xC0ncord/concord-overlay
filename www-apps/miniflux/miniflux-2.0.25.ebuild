...
EGO_SUM=(
	...
	"github.com/tdewolff/minify/v2 v2.9.10/go.mod"
	"github.com/tdewolff/parse/v2 v2.5.5"
	"github.com/tdewolff/parse/v2 v2.5.5/go.mod"
...
FILECAPS=(
	cap_net_bind_service+ep usr/bin/miniflux
)
...
src_install() {
	...
	insinto /etc
	newins "${FILESDIR}/miniflux.conf" miniflux.conf

	if use acct; then
		cp "${FILESDIR}/miniflux.initd-r1" "${S}"
		sed -i 's/root/miniflux/' "${S}/miniflux.initd-r1"
		newinitd "${S}/miniflux.initd-r1" miniflux
	else
		newinitd "${FILESDIR}/miniflux.initd-r1" miniflux
	fi
...
pkg_postinst() {
	...
	einfo "Before using Miniflux, you must first create and initialize the database:"
	einfo "# su - postgres"
	einfo "$ createuser -P miniflux"
	einfo "$ createdb -O miniflux miniflux"
	einfo ""
	einfo "The hstore extension should be enabled for the Miniflux database. You may:"
	einfo "1. Give SUPERUSER privileges to the miniflux user only during schema migration:"
	einfo "-> psql> ALTER USER miniflux WITH SUPERUSER;"
	einfo "-> -- Run the migrations (miniflux -migrate)"
	einfo "-> psql> ALTER USER miniflux WITH NOSUPERUSER;"
	einfo ""
	einfo "2. Create the hstore extension with another user that has SUPERUSER privileges:"
	einfo "   before running the migrations."
	einfo "-> sudo -u postgres psql miniflux"
	einfo "-> psql> CREATE EXTENSION HSTORE;"
	einfo ""
	einfo "If you are upgrading from a previous version, you must migrate the database:"
	einfo "1. Export the environment variable DATABASE_URL if not already done"
	einfo "2. Disconnect all users by flushing all sessions: miniflux -flush-sessions"
	einfo "3. Stop miniflux"
	einfo "4. Backup your database"
	einfo "5. Check that your backup is really working"
	einfo "6. Run database migrations: miniflux -migrate or set the environment variable RUN_MIGRATIONS=1"
	einfo "7. Start miniflux again"
}
